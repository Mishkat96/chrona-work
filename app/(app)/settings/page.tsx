...
import {
  ...
  Users, UserPlus, Copy, Check, Trash2, AlertCircle, CheckCircle2, X,
} from "lucide-react";
...
function ProfileSection() {
  const { currentUser, updateUserProfile } = useTasks();
  ...
  // Sync fields if currentUser loads after mount
  useEffect(() => {
    if (currentUser) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      await updateUserProfile({ name: name.trim(), jobTitle, department, initials });
      setSaved(true);
  ...
  const inputClass = "w-full h-9 px-3 rounded-lg border border-border bg-white text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors";
      ...
      <CardContent className="space-y-5">
        ...
        <form onSubmit={handleSave} className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input value={currentUser?.email ?? ""} readOnly className="w-full h-9 px-3 rounded-lg border border-border bg-muted/40 text-sm text-muted-foreground cursor-not-allowed" />
          </div>
          <div>
...
function WorkspaceSection() {
  const { currentUser, workspaceName, updateWorkspace } = useTasks();
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={!isAdmin}
              required
              className="w-full h-9 px-3 rounded-lg border border-border bg-white text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-colors disabled:bg-muted/40 disabled:cursor-not-allowed"
            />
          </div>
          {isAdmin && (
            <Button type="submit" disabled={saving}>
    ...
    <div className="flex gap-6 max-w-[1200px] h-[calc(100vh-88px)]">
      ...
      <div className="flex-1 overflow-y-auto space-y-5">
        ...
        {activeSection === "notifications" && (
          <ComingSoon
            title="Notifications"
            desc="Notification preferences will be available in an upcoming release."
          />
        )}
        {activeSection === "security" && (
          <ComingSoon
            title="Security"
            desc="Two-factor authentication, SSO, and audit logs are coming soon."
          />
        )}
        {activeSection === "ai" && (
          <ComingSoon
            title="AI & Automation"
            desc="AI scheduling preferences and automation rules are coming soon."
          />
        )}
        {activeSection === "billing" && (
          <ComingSoon
            title="Billing"
            desc="Billing and subscription management will be available soon. Contact us to manage your plan."
          />
        )}
        {activeSection === "integrations" && (
          <ComingSoon
            title="Integrations"
            desc="Slack, Google Calendar, GitHub, and other integrations are coming soon."
          />
        )}
      </div>
    </div>
